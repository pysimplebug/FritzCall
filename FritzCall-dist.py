...
TCP_IP = '192.168.1.1' # Fritzbox IP
TCP_PORT = 1012 # Port for the CallMonitor Service (default 1012)
PASS='put_your_password_here' # Password of the fritz phonebook user (or admin)
BUFFER_SIZE = 1024 # Buffer size (no need to change)
data = "No call yet"
...
DEFAULT_DISPLAY_DURATION_IN_MILLISECONDS = 8000
    ...
    while True:
        ...
        if 'CALL' in calldata[1] or 'RING' in calldata[1]:
            ...
            if number in contacts:
                display_notification(direction, contacts.get(number, "none") + ' (' + number + ')', img_success, 6000, use_fade_in=True)
            else:
                display_notification(direction, number, img_success, 6000, use_fade_in=True)
